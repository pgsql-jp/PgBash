 ...
 *
 * Change Logs
 * 1. Add the exec_sql_init() call.
 * 2. Add bind_xxx_variable() wrap routine.
 * 3. Add the parameter into bind_variable(). (2011/08/10)
 * 4. Add wrap_bind_array_variable(). (2011/08/15)
 *
 *======================================================================
 */
...
#  endif
 ...
 /*==============================================
   * Initialize exec_sql function for PostgreSQL
   *=============================================*/
  exec_sql_init(interactive_shell);
...
/*
 * ################################################################### 
 * bash shell bind_variable() wrap routine.
 * ##################################################################
 */

int
wrap_bind_int_variable(char *name, int value)
/*******************************************************************
 *   bind integer variable to shell variable
 *******************************************************************/
{
	SHELL_VAR      *var;
	char            buf[32];
	...
	if ((var = bind_variable(name, buf, 0)) == NULL)
		return (-1);
	//stupidly_hack_special_variables (name);
...
int 
wrap_bind_char_variable(char *name, char *value)
/*******************************************************************
 *   bind_variable wrapper routine.
 *******************************************************************/
{
	SHELL_VAR      *var;

	if ((var = bind_variable(name, value, 0)) == NULL)
		return (-1);
	//stupidly_hack_special_variables (name);
...
int 
wrap_find_variable(char *name, char *value)
/*******************************************************************
 *   find_variable wrapper routine.
 *******************************************************************/
{
	SHELL_VAR	*var;
	...
	if ((var = find_variable(name)) != NULL )
...
int 
wrap_bind_array_variable(char *name, int dummy, char *value, int flags)
/*******************************************************************
 *   bind_array_variable wrapper routine.
 *******************************************************************/
{
	char		*newval;
	char		*value2, *p1, *p2, *p3;
	int			i;

//printf("# wrap_bind_array_variable(): name=(%s) value=(%s)<br>\n", name, value);

	value2 =  (char *)xmalloc (strlen(value)+8);
	...
	/*----- make array string -----*/
	newval = (char *)xmalloc (strlen(name)+strlen(value2)+32);

	sprintf(newval, "\\\"${%s[@]}\\\" \\\"%s\\\"", name, value2);

//printf("1.wrap_bind_array_variable(): newval=(%s)<br>\n", newval);

	(void)assign_array_from_string (name, newval, flags);
