...
{
       0,   389,   389,   400,   409,   424,   434,   436,   440,   446,
     452,   458,   464,   470,   476,   482,   488,   494,   500,   506,
     512,   518,   524,   530,   537,   544,   551,   558,   565,   572,
     578,   584,   590,   596,   602,   608,   614,   620,   626,   632,
     638,   644,   650,   656,   662,   668,   674,   680,   686,   692,
     698,   704,   712,   714,   716,   720,   724,   735,   737,   741,
     743,   745,   761,   763,   767,   769,   771,   773,   775,   777,
     779,   781,   783,   785,   787,   791,   796,   801,   806,   811,
     816,   821,   826,   833,   838,   843,   848,   855,   860,   865,
     870,   875,   880,   887,   892,   897,   904,   907,   910,   914,
     916,   947,   954,   959,   976,   981,   998,  1005,  1007,  1009,
    1014,  1018,  1022,  1026,  1028,  1030,  1034,  1035,  1039,  1041,
    1043,  1045,  1049,  1051,  1053,  1055,  1057,  1059,  1063,  1065,
    1074,  1082,  1083,  1089,  1090,  1097,  1101,  1103,  1105,  1112,
    1114,  1116,  1120,  1121,  1124,  1126,  1128,  1132,  1133,  1142,
    1155,  1171,  1186,  1188,  1190,  1197,  1200,  1204,  1206,  1212,
    1218,  1224,  1230,  1250,  1252,  1275,  1279,  1281
};
#endif
...
/*=========================================
 *   Change "select" to "selects" for SQL.
 *========================================*/
  { "selects", SELECT },
#endif
  { "while", WHILE },
...
yylex ()
{
 ...
 /*===============================================
  * Change read_token() to read_EXEC_SQL_token()
  * Original: current_token = read_token (READ);
  *=============================================*/
 (void)read_EXEC_SQL_token();  /* return current_token */
...
/*==================================================================
 * Change "SQL;" string to 'exec_sql' token and one "SQL" token.
 *
 * If token is SQL reserved word, then this prase_SQL_statement starts.
 * And it changes string to 'exec_sql "SQL"'.
 *
 *  These routines have been made by SAKAIDA and KUBO-san.
 *  SAKAIDA Masaaki                  at 1999.11.11 -- Osaka,Japan
 *  KUBO Takehiro <takehiro@kubo.ws> at 2000.11.12 -- Kanagawa,Japan
 *
 * Change Logs 
 * 2000.11.12: Modify fix_sized array to use RESIZE_MALLOCED_BUFFER() (KUBO)
 * 2001.04.15: Modify source style. (SAKAIDA)
 * 2001.04.15: Add SQL_word[] to "CHECKPOINT". (SAKAIDA)
 * 2001.04.15: Change SQL_MAXLEN to 65536. (SAKAIDA)
 * 2001.04.15: Change '\t' to ' '. (SAKAIDA)
 * 2001.08.01: Fix the bug of parsing ';' surrounded in parenthesis.(SAKAIDA)
 * 2001.11.26: Add 'ANALYZE' of SQL_word[].
 * 2002.05.09: Change local variables to static variable in read_EXEC_SQL_token().
 * 2002.05.10: Add the functionality of describing the '#' comment in SQL.
 * 2002.07.15: Add "REINDEX" into SQL_word[]. (ISHIDA akio)
 * 2002.07.17: Fix the bug of the double quotation surrounded by the single quotation
 *             and the bug of the single quotation surrounded by the double quotation.
 * 2002.07.17: Add the functionality of processing the signle quotation data
 *             surrounded by the special signle quotation.
 * 2011.08.10: Add SQL reserved words.
 *
 *==================================================================
 */
#include <string.h> 

#define SQL_EXEC_WORD	"exec_sql"
#define SQL_MAXLEN	65536       /* max. length for check */
...
char *SQL_word[] = {
	...
	"COPY",
	"CREATE",       "DEALLOCATE",	"DECLARE",      "DELETE",
	"DISCARD",		"DISCONNECT",   "DROP",
	...
	"NOTIFY",
	"PREPARE",
	"REASSIGN",		"REINDEX",      "RELEASE",		"RESET",
	"REVOKE",       "ROLLBACK",
	"SAVEPOINT",	"SELECT",       "SET",          "SHOW",
	"START",
	"TABLE", 		"TRUNCATE",
	"UNLISTEN",     "UPDATE",
	"VACUUM",		"VALUES",
...
static int read_EXEC_SQL_reserved_word(char *reserved, int *len);
static int read_EXEC_SQL_statement(char *reserved, int len);
 ...
 * %token       TIME, TIMEOPT, WORD
 *
 * return current_token (global variable)
 *******************************************************************/
{
static	int	SQL_flag = 0;	/* if a token is SQL then 1 else 0 */
static	char	reserved[32];	/* e.g. "\"SELECT " */
static	int	len = 0;	/* reserved variable length */
	...
	if (SQL_flag == 0)  
	{
		...
		/*------ serach SQL reserved word ------*/
		SQL_flag = read_EXEC_SQL_reserved_word(reserved, &len);
	}
	else if(SQL_flag == 1)        
	{
		/*--- change tokens to one SQL token ---*/
		(void)read_EXEC_SQL_statement(reserved, len);
		SQL_flag = 0 ;
	}
}

static int
read_EXEC_SQL_reserved_word(char *reserved, int *len)
/********************************************************************
 *  search SQL reserved word.
 *******************************************************************/
{
	int	SQL_flag = 0;
	int	len1, len2;
	...
	*len = 0;
	...
	{
		index= 0;
		len1 = strlen(yylval.word->word);

		while( SQL_word[index] != NULL )
		{
			len2 = strlen(SQL_word[index]);

		        /*------- Compare WORD and SQL_start_word --------*/
			if( len1==len2 && strUcmp(yylval.word->word,&SQL_word[index][0],len1)==0 )
			{
				/*----- Stock SQL reserved word ----------*/
				strcpy( reserved, "\"" );
				strcat( reserved, yylval.word->word );
				strcat( reserved, " " );
				*len = strlen(reserved);

				SQL_flag = 1 ;

				/*------- Change Token to 'exec_sql' -----*/
				free( yylval.word->word );
				yylval.word->word = xmalloc( sizeof(SQL_EXEC_WORD) + 1 );
				strcpy( yylval.word->word, SQL_EXEC_WORD);
				break;
			}
			index++;
		}
...
static int
read_EXEC_SQL_statement(char *reserved, int len)
/*******************************************************************
 *  change some tokens to one SQL token surrounded by double quote.
 ******************************************************************/
{
	WORD_DESC	*the_word;
	int		character, character2;
	int		sqt = 0, wqt = 0, ssqt = 0;
	int		retsize = TOKEN_DEFAULT_GROW_SIZE;
	int		retind  = len;
	int		parenth = 0;

	/*---------- Setting of parser variable ------------*/
	the_word = (WORD_DESC *)xmalloc (sizeof (WORD_DESC));
	the_word->word = (char *)xmalloc(TOKEN_DEFAULT_GROW_SIZE);
	the_word->flags = 0;
	...
	/*---------- Initial settings ----------------------*/
	strcpy(the_word->word, reserved);
	retind = len;

	while ((character = shell_getc (1)) != EOF)
	{
		character2 = '\0';

		/*--------------- stock  character ----------------------*/
		RESIZE_MALLOCED_BUFFER (the_word->word, retind, 1, retsize, 
						TOKEN_DEFAULT_GROW_SIZE);
		the_word->word[retind++] = (char)character;

		/*-------------------- escape character -----------------*/
		if( character == '\\' )
		{
		        character2 = shell_getc (1);
			RESIZE_MALLOCED_BUFFER (the_word->word, retind, 1, retsize, 
							TOKEN_DEFAULT_GROW_SIZE);
			the_word->word[retind++] = (char)character2;

			/*-------- special single quote --------*/
			if( wqt == 0 && sqt == 0 && ssqt == 0 && character2 == '\'' ) 
				ssqt = 1;
			else if( wqt == 0 && sqt == 0 && ssqt == 1 && character2 == '\'' ) 
				ssqt = 0;

			/*------------ double quote ------------*/
			if( sqt == 0 && wqt == 0 && character2 == '\"' ) 
				wqt = 1;
			else if( sqt == 0 && wqt == 1 && character2 == '\"' ) 
				wqt = 0;
		...
		if( ssqt == 1 && character == '"' )   /* \'..."...\' */
		{
			RESIZE_MALLOCED_BUFFER (the_word->word, retind, 1, retsize, 
							TOKEN_DEFAULT_GROW_SIZE);
			the_word->word[retind-1] = '\\';
			the_word->word[retind++] = (char)character;
			continue;
		}

		if ( ssqt == 1 )   /* characters between special single quote */
			continue;


		/*-------------------- single quote ---------------------*/
		if( wqt == 0 && sqt == 0 && character == '\'' )   /* start */
		{
			sqt = 1;
			continue;
		}

		if( wqt == 0 && sqt == 1 && character == '\'' )   /* end of single quote */
		{
		        character2 = shell_getc (1);
			RESIZE_MALLOCED_BUFFER (the_word->word, retind, 1, retsize, 
							TOKEN_DEFAULT_GROW_SIZE);
			...
			character = character2;      /* end of single quote */
			sqt = 0;  
		}

		if( wqt == 0 && sqt == 1 && character == '"' )   /* '...\"...' */
		{
			RESIZE_MALLOCED_BUFFER (the_word->word, retind, 1, retsize, 
							TOKEN_DEFAULT_GROW_SIZE);
			the_word->word[retind-1] = '\\';
		...
		if ( wqt == 0 && sqt == 1 )	   /* characters between single quote */
			continue;

		/*-------------------- double quote ---------------------*/
		if( sqt == 0 && wqt == 0 && character == '\"' )   /* start */
		{
			RESIZE_MALLOCED_BUFFER (the_word->word, retind, 1, retsize, 
							TOKEN_DEFAULT_GROW_SIZE);
			the_word->word[retind-1] = '\\';
		...
		if( sqt == 0 && wqt == 1 && character == '\"' )   /* end of single quote */
		{
			RESIZE_MALLOCED_BUFFER (the_word->word, retind, 1, retsize, 
							TOKEN_DEFAULT_GROW_SIZE);
			the_word->word[retind-1] = '\\';
		...
		if ( sqt == 0 && wqt == 1 )    /* characters between double quote */
			continue;

		/*--------------- Comment -------------------------------*/
		if( sqt == 0 && wqt == 0 && character == '#' )
		{
			the_word->word[retind-1] = ' '; 
			while ((character = shell_getc (1)) != EOF)
			{
		...
		if( sqt == 0 && wqt == 0 && parenth == 0 && character == ';' )
			break;
	
		/*--------------- chaeck max length ---------------------*/
		if( retind > SQL_MAXLEN )
		{
    			fprintf (stderr, "Error: SQL statement too long(max.:%d).\n", SQL_MAXLEN);
			break;
		}
	}

	RESIZE_MALLOCED_BUFFER (the_word->word, retind, 2, retsize, 
					TOKEN_DEFAULT_GROW_SIZE);
	the_word->word[retind++] = '"';
	the_word->word[retind++] = '\0';
